...
//subtract one from the total amount of servo boards (done to save on confusion where servoboard 0 is a single board)
...
#define LPN 36      //lowest playable midi note
#define HPN 81      //highest playable midi note
//end inclusions
...
void MidiNoteOn(byte channel, byte note, byte velocity)   //function called when receving a note
{
  ...
  bool Procede = false;
  if (note >= LPN and note <= HPN)                      //test for playable notes
  {
    DEBUG_PRINTLN("Note >= LPN and Note <= HPN");
    //mapping for the base side
    if(channel == LHC and note <= 71)   //move the notes for the bass side and make sure theyre in the bass range
    {
      DEBUG_PRINTLN("channel == LHC and note <= 71");
      if(CLHV == 1)//full range of 36 to 59
      {
        DEBUG_PRINTLN("CLHV == 1");
        note = note%12;
        Procede = true;
      }
      else if (note >= 48 and note <= 59)
      {
        DEBUG_PRINTLN("note >= 48 and note <= 59");
        note = note%12;
        Procede = true;
      }
      ...
      //Servo 48 is the starting servo for the bass side
      note = note + 48;
    }
    else if (note >= LPN+5)
    {
      DEBUG_PRINTLN("note >= LPN+5");
      //add five because the lowest playable note on the piano side is 41
      note = note - LPN+5;              //subtract out the offset of playable notes
      Procede = true;
    }
   ...
   //below this can be removed if the instrument has little delay in making a sound and stopping that sound
      if(IOEOffByteTimer[note] > 0)                       //check if the notes are coming in faster than they can be played
      {                                                   //if they are cut the note short
        DEBUG_PRINTLN("Notes Too Fast");
        DEBUG_PRINT("IOEOffByteTimer[");DEBUG_PRINT(note);DEBUG_PRINT("] = ");DEBUG_PRINTLN(IOEOffByteTimer[note]);
        IOEOffByteTimer[note] = IOEOffByteTimer[note] - IOEOnByteTimerModifer[note];  //cut the last note short by the on time
        IOEOnByteTimer[note] = CurrentMillis + IOEOnByteTimerModifer[note] + 10;      //start the new note slightly late
        return;
      }
      IOEOnByteTimer[note] = CurrentMillis + IOEOnByteTimerModifer[note];
      DEBUG_PRINT("CT:");DEBUG_PRINT(CurrentMillis);DEBUG_PRINT(" IOEOnByteTimer[");DEBUG_PRINT(note);DEBUG_PRINT("] = ");DEBUG_PRINTLN(IOEOnByteTimer[note]);
      IOEOffByteTimer[note] = CurrentMillis + 20000;   //this is just a saftey to stop solenoids from burning out
    }
  }
}
...
void MidiControlChange(byte channel, byte control, byte value)  //other midi commands
{
  ...
  switch (control)              //test to see what command is being sent
  {
    case 123:                   //all notes off command
      for (int i; i <= 5; i++)  //just clear out the buffer
      {
        MIDI.read(LHC);
        MIDI.read(RHC);
        delay(5);
